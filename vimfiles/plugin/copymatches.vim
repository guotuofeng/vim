" "============================================================================
" File:          copymatches.vim
" Author:        Mike Guo (myguo@microsoft.com)
" Briefs:        Gather search hits and display in a new scratch buffer
" Last Change:   2011/06/24 09:20:10
"
" Note: 
"       Copied from VIM wiki by myguo
"       Plugin to copy matches (search hits which may be multiline).
"       Version 2012-05-03 from http://vim.wikia.com/wiki/VimTip478

if exists("loaded_copymatches")
  finish
endif

let loaded_copymatches=1
...
function! s:CopyFirstMatch(line1, line2, reg)
  let hit=""
  for line in range(a:line1, a:line2)
    ...
    while idx >= 0
      ...
      if end > idx
        let hit=strpart(txt, idx, end-idx)
        break
      else
        let end += 1
      endif
...
command! -range=% -register CopyFirstMatch call s:CopyFirstMatch(<line1>, <line2>, '<reg>')

nnoremap <unique> <leader>g :CopyFirstMatch<CR>
